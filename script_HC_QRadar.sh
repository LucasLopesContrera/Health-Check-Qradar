...
## Exemplo de Uso: Executar o arquivo scriptColeta_HC.sh, após o término da execução, no mesmo diretório será criado uma pasta "Resultado_HC_Leadcomm.tar.gz".
...
#                                            DECLARAÇÃO DE FUNÇÕES
 ...
 function verificaSeArquivoJaExiste () {
  ...
  echo
  echo "Preparando para iniciar, Aguarde!"
  fi
 }
...
if [ $VERIFICA != 0 ]; then
echo "O seu usuário é ${USUARIOATUAL}. Precisa executar com root…"
exit 1
else
echo "Iniciando, aguarte o término da execução! "
fi
...
#                                                 APPLIANCE
 ...
 function apl_02 () {
   echo
   echo
   echo -e '    Nome do teste     :  Tempo total do Servidor Ligado'
...
function apl_11 () {
   echo
   echo
   echo '    Nome do teste    :  Verificar os últimos Backups'   
...
function apl_12 () {
   echo
   echo
   echo '    Nome do teste    :  Uso de CPU data Comando SAR'   
   echo -e '    Saída do comando :\n\n ';  sar -P ALL 5 2 ; status 
   echo
...
function apl_13 () {
   echo
   echo
   echo '    Nome do teste    :  15 processos que mais consomem CPU'   
   echo -e '    Saída do comando :\n\n ';  ps -ef | egrep -v "STIME|$LOGNAME"  ; status 
   echo
...
main_appliance | tee -a appliance.txt
...
main_operacao | tee -a operacao.txt


##################################################################################################################
#                                         CRIAÇÃO DO ARQUIVO DE COLETA

echo CRIANDO ARQUIVO Resultado_HC_Leadcomm.tar.gz

mkdir -p Resultado_HC_Leadcomm/ ; mv appliance.txt coleta.txt operacao.txt Resultado_HC_Leadcomm/ ; tar -czvf Resultado_HC_Leadcomm.tar.gz Resultado_HC_Leadcomm/ ;  rm -rf Resultado_HC_Leadcomm/
